...
/* ================================================================ */
/* This functions retrieves OpenCL compatible devices and choose a 
	 GPU type device for use
 */
...
cl_kernel initKernel (int cacheSize, int vecSize, cl_context ctx)
{
	...
	if(vecSize == 1) {
		...
		strcpy (temp, "__kernel void clCache(__global float4* output, __local float4* sm) {\n");
	} else if (vecSize == 8) {
...
void runTest (int nThreads, int bSize, int cacheSize, int vecSize)
{
	...
	/* Block until all queued kernels have completed */
	...
	/* Collect and accumulate various timing information */
	cl_ulong timeAccum = 0;
	cl_ulong timeStart, timeEnd;
	for(i = 0; i < NITER; i++) {
		/* Collect kernel start times */
